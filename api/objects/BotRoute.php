...
class BotRoute{
    ...
    function getarrays(){
        // select all query
        $query = "SELECT blockade FROM " . $this->table_name . " WHERE BOT_ID = ".$this->BOT_ID."";
    ...
    function setblockade(){
        ...
        while ($row = $getarray->fetch(PDO::FETCH_ASSOC)){
            $blocade = json_decode($row['blockade']);
        }


        array_push($blocade, $this->blockade);
        ...
        // bind values
        $stmt->bindValue(":blockade", json_encode($blocade));
